...
class TennisComparisonEngine:
    ...
    def validate_user_video(self, video_path: str, user_metadata: Dict) -> Dict:
        ...
        try:
            ...
            # [OK] USAR CLASSIFICADOR REAL
            try:
                print("[ROBOT] Executando classificador biomecanico...")
                
                # Executar classificador real via subprocess
                result = subprocess.run([
                    sys.executable, 'improved_biomech_classifier_2d.py', video_path
                ], capture_output=True, text=True, timeout=30)

                if result.returncode == 0:
                    # [OK] PARSEAR OUTPUT DO CLASSIFICADOR REAL
                    ...
                    print(f"[ERRO] ERRO no classificador: {result.stderr}")
                    raise Exception(f"Classificador falhou: {result.stderr}")

            except subprocess.TimeoutExpired:
                raise Exception("Timeout na analise do video")
            except Exception as e:
                raise Exception(f"Falha na analise do video: {e}")
    ...
    def _parse_classifier_output(self, output: str) -> Dict:
        ...
        try:
            ...
            # 1. MOVIMENTO (buscar por "TENNIS Movimento: forehand_drive")
            movement_patterns = [
                r'TENNIS Movimento:\s*(\w+)',
                r'Movimento:\s*(\w+)',
                r'TENNIS Tipo de movimento:\s*(\w+)'
            ]
            
            for pattern in movement_patterns:
                movement_match = re.search(pattern, output, re.IGNORECASE)
                if movement_match:
                    detected_info['movement_type'] = movement_match.group(1)
    ...
    def _build_expected_info(self, metadata: Dict) -> Dict:
        ...
        # Mao dominante
        expected['dominant_hand'] = metadata['maoDominante']  # D ou E
        
        # Lado da camera  
        expected['camera_side'] = metadata['ladoCamera']  # D ou E
    ...
    def compare_techniques(self, user_video_path: str, professional_video_path: str,
        ...
        try:
            ...
            if self.cycle_analyzer and self.analysis_interface:
                ...
                # VALIDACAO CRITICA 1: Verificar se configuracao bate com arquivo do usuario
                user_file_movement = self._extract_movement_from_professional(user_video_path, {})
                user_config_movement = validated_params['movement_type']
                
                print(f"[VALIDATION] Arquivo usuario: {user_file_movement}")
                print(f"[VALIDATION] Config usuario: {user_config_movement}")
                
                if user_file_movement != 'unknown' and user_file_movement != user_config_movement:
                    error_msg = f"INCONSISTENCIA INTERNA: Arquivo={user_file_movement}, Config={user_config_movement}"
                    logger.error(f"[CONFIG_MISMATCH] {error_msg}")
                    print(f"[REJECTED] {error_msg}")
                    return {
                        'success': False,
                        'error': f'Configuracao inconsistente: {error_msg}',
                        'validation_failed': True,
                        'config_mismatch': True,
                        'file_movement': user_file_movement,
                        'config_movement': user_config_movement,
                        'final_score': 0,
                        'analysis_type': 'config_validation_failed',
